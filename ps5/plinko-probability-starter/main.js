...
const probabilityInput = document.querySelector('#rightward-probability'); // The input for the probability of a ball going right
...
async function animateBallDrop(circle, targetY, duration) {
    const initialY = parseFloat(circle.getAttribute('cy'));
    ...
    return new Promise(resolve => {
        const animate = (time) => {
            ...
            const newY = initialY + easeInQuad(progress) * (targetY - initialY);
...
function drawBoard() {
    ...
    PROBABILITY_RIGHT = parseFloat(probabilityInput.value);
    ...
    for(let level = 0; level < NUM_LEVELS; level++) {
        ...
        for(let i = NUM_LEVELS - level - 1; i <= NUM_LEVELS + level - 1; i+=2) {
            ...
            const initialPegColor = '#FEFEFE'; // initial color, or use any other color
            const circle = createCircle(x, y, PEG_RADIUS, initialPegColor, 'none', svgElement);
            if(level === 0) {
                const topPegColor = chroma.scale(['#2F65A7', 'darkblue'])(1).hex();
                circle.setAttribute('fill', topPegColor);
    ...
    function updatePegColor(row, col) {
        const scale = chroma.scale(['#cccccc', 'darkred']).domain([0, 1]); // Define the color scale from light gray to dark red
        const hitCountRatio = hitCounts[row][col] / NUM_BALLS; // Calculate the hit count ratio
        const color = scale(hitCountRatio).hex(); // Get the color from the scale
        const peg = pegs[row][col];
        peg.setAttribute('fill', color);
    }
    ...
    async function dropBall() {
        ...
        const ballColor = chroma(BALL_COLOR).darken(2 * Math.random() - 1).saturate(2 * Math.random() - 1).hex(); // Random initial color for the ball
        ...
        for(let i = 0; i < NUM_LEVELS-1; i++) {
            ...
            await moveCircleTo(circle, x, y, DELAY_BETWEEN_PEGS / parseFloat(speedInput.value), easeOutQuad); // Pass the easeOutQuad function as the easing function
        ...
        await Promise.all([
            changeHeightTo(actualBars[barIndex], newBarHeight, DELAY_WHEN_DROP / parseFloat(speedInput.value)), // Animate the change in height of the bar
            animateBallDrop(circle, y + GRAPH_HEIGHT, 3 * DELAY_WHEN_DROP / parseFloat(speedInput.value)) // Animate the ball dropping down (duration tripled)
        ]);
    ...
    async function dropBalls() {
        ...
        numBallsInput.setAttribute('disabled', true); // Disable the new input
        probabilityInput.setAttribute('disabled', true); // Disable the new input

        const numBallstoDrop = parseInt(numBallsInput.value); // Update the number of balls based on user input
        const dropBallPromises = []; // An array of promises for each ball that is dropped
        PROBABILITY_RIGHT = parseFloat(probabilityInput.value); // Update the probability based on user input

        for(let i = 0; i < numBallstoDrop; i++) {
        ...
        numBallsInput.removeAttribute('disabled'); // Re-enable the new input
        probabilityInput.removeAttribute('disabled'); // Re-enable the new input
    }
    ...
    probabilityInput.addEventListener('input', redrawBoard);
...
// Animates the movement of a circle to a new location with easing function
async function moveCircleTo(circle, cx, cy, duration, easingFunction) {
